...
export default async function LateWorkEmailApprovalPage({
  searchParams,
}: {
  ...
  const { request, error } = await getLateWorkRequestById(parsed.data.requestId);
  if (!request) {
    ...
    <div style={{ minHeight: "100vh", background: "#f4f6f8", padding: 28 }}>
      <div style={{ maxWidth: 760, margin: "0 auto", background: "#fff", borderRadius: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.05)", padding: 28 }}>
        ...
        <h1 style={{ margin: "8px 0 0", fontSize: 28, fontWeight: 800, color: "#111" }}>
          {parsed.data.decision === "APPROVE" ? "Approve late work" : "Reject late work"}
        </h1>
        ...
        <EmailApprovalForm
          ...
          executePath="/api/email-approval/late-work/execute"
          entityLabel="late work request"
        />

        <div style={{ marginTop: 18 }}>
          <Link href={`/late-work/${request.id}`} style={{ color: "#0f172a", fontWeight: 700 }}>
            Open the late work request in the app
          </Link>
        </div>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f6f8", padding: 28 }}>
      <div style={{ maxWidth: 760, margin: "0 auto", background: "#fff", borderRadius: 18, boxShadow: "0 2px 8px rgba(0,0,0,0.05)", padding: 28 }}>
        ...
        <Link href="/late-work/dashboard" style={{ color: "#0f172a", fontWeight: 700 }}>
          Return to dashboard
        </Link>
      </div>
    </div>
