...
import { updateShutdown } from "@/lib/shutdown/setup";
...
export async function POST(req: Request) {
  ...
  try {
    const shutdown = await updateShutdown({
      id: String(input.id || ""),
    ...
    if (isFormPost) {
      return NextResponse.redirect(
        new URL(`/admin/shutdowns/${shutdown.id}?updated=1`, req.url),
        { status: 303 }
      );
    }
    ...
    const target = id ? `/admin/shutdowns/${id}` : "/admin/shutdowns";

    if (isFormPost) {
      return NextResponse.redirect(
        new URL(`${target}?error=${encodeURIComponent(message)}`, req.url),
        { status: 303 }
      );
    }
