...
import { notifyRemovalStageApprovers } from "@/lib/email/removal-notifications";
...
export async function POST(req: Request) {
  try {
    ...
    if (cleanLinesInput.length === 0 || cleanLinesInput.length !== resources.length) {
      return NextResponse.json(
        { error: "At least one removal resource is required, and each line must include hours greater than 0." },
        { status: 400 }
      );
    }
    ...
    const { error: linesErr } = await supabase.from("work_removal_resources").insert(cleanLines);
    ...
    const actor =
      ...
      "Unknown";
    const request = header as WorkRemovalRequestRecord;

    let emailWarning: string | undefined;
    const emailResult = await notifyRemovalStageApprovers(request, "SUBMITTED", actor).catch((error) => ({
      attempted: true,
      sent: false,
