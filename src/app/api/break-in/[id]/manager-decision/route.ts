...
export async function POST(
  ...
  if (!canApproveStage(auth.user, "MANAGER_REVIEW")) {
    return NextResponse.json({ error: getApprovalPermissionError("MANAGER_REVIEW") }, { status: 403 });
  }

  const actor = auth.user?.full_name || auth.user?.email || "A manager";
