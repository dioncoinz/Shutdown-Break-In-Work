...
export async function POST(
  ...
  try {
    await deleteRequestWithAudit({
      ...
      resourceTable: "late_work_resources",
      table: "late_work_requests",
    });

    return NextResponse.redirect(new URL("/late-work/dashboard?deleted=1", req.url), { status: 303 });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to delete request.";
    return NextResponse.redirect(
      new URL(`/late-work/${id}?deleteError=${encodeURIComponent(message)}`, req.url),
      { status: 303 },
    );
  }
}
