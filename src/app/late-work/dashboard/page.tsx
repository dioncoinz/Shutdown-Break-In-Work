...
export default async function LateWorkDashboardPage({
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f6f8", display: "grid", gridTemplateColumns: "176px minmax(0, 1fr)" }}>
      ...
      <form method="GET" action="/late-work/dashboard" style={{ marginTop: 18, display: "flex", justifyContent: "flex-end", gap: 8, flexWrap: "wrap", alignItems: "end" }}>
      ...
      <div style={{ marginTop: 20, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16 }}>
        ...
        <KpiLink href={dashboardHref("APPROVED", selectedShutdownId, searchQuery)} active={filter === "APPROVED"} label="Approved Late Work" value={approvedRows.length} color="#2563eb" />
          ...
          <tbody>
            {filteredRows.map((row, idx) => (
              <tr key={row.id} style={{ borderBottom: "1px solid #eee", background: idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                ...
                <Td><Link href={`/late-work/${row.id}?returnTo=${encodeURIComponent(dashboardHref(filter, selectedShutdownId, searchQuery))}`} style={openButtonStyle}>Open</Link></Td>
              </tr>
            ))}
...
function dashboardHref(filter: string, shutdownId: string | null, searchQuery: string) {
  ...
  return query ? `/late-work/dashboard?${query}` : "/late-work/dashboard";
}
