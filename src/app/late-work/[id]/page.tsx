...
export default async function LateWorkDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const approvalStages = [
    { title: "Planner review", activeStatuses: ["SUBMITTED"], doneStatuses: ["COORD_REVIEW", "SUPER_REVIEW", "APPROVED", "REJECTED"], comment: request.planner_comment, completedBy: request.planner_decided_by, completedAt: request.planner_decided_at },
    { title: "Shutdown Coordinator review", activeStatuses: ["COORD_REVIEW"], doneStatuses: ["SUPER_REVIEW", "APPROVED", "REJECTED"], comment: request.coordinator_comment, completedBy: request.coordinator_decided_by, completedAt: request.coordinator_decided_at },
    { title: "Superintendent review", activeStatuses: ["SUPER_REVIEW"], doneStatuses: ["APPROVED", "REJECTED"], comment: request.superintendent_comment, completedBy: request.superintendent_decided_by, completedAt: request.superintendent_decided_at },
  ] as const;
    ...
    <div style={{ padding: 28, background: "#f4f6f8", minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <Link href="/late-work/new" style={primaryButton}>+ New late work</Link>
          <Link href="/late-work/dashboard" style={secondaryButton}>Late Work Dashboard</Link>
        </div>
      </div>
