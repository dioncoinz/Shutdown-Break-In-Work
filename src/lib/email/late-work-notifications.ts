...
function buildRequestUrl(requestId: string) {
  ...
  return `${baseUrl}/late-work/${requestId}`;
}
...
function buildEmailApprovalUrl({
  ...
  return `${baseUrl}/email-approval/late-work?token=${encodeURIComponent(token)}`;
}
...
function formatRequestSummary(request: LateWorkRequestRecord) {
  return `
...
function formatRequestSummaryText(request: LateWorkRequestRecord) {
  return [
...
export async function notifyLateWorkStageApprovers(
  request: LateWorkRequestRecord,
  status: Extract<LateWorkStatus, "SUBMITTED" | "COORD_REVIEW" | "SUPER_REVIEW">,
  triggeredBy: string,
) {
      ...
      return `
        <tr>
          ...
          <td style="padding: 8px 0; border-top: 1px solid #e5e7eb;">
            <div><a href="${approveUrl}">Approve late work</a></div>
            <div style="margin-top: 6px;"><a href="${rejectUrl}">Reject late work</a></div>
          </td>
        </tr>
      `;
    })
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: Arial, Helvetica, sans-serif; color: #111; line-height: 1.5;">
        <p><strong>Late work approval required</strong></p>
...
export async function notifyLateWorkRequestorOutcome(
  request: LateWorkRequestRecord,
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: Arial, Helvetica, sans-serif; color: #111; line-height: 1.5;">
        <p><strong>Late work request ${escapeHtml(title)}</strong></p>
