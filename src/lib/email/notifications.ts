...
function buildRequestUrl(requestId: string) {
  ...
  return `${baseUrl}/break-in/${requestId}`;
}
...
function buildEmailApprovalUrl({
  ...
  return `${baseUrl}/email-approval?token=${encodeURIComponent(token)}`;
}
...
function formatRequestSummary(request: BreakInRequestRecord) {
  return `
...
function formatRequestSummaryText(request: BreakInRequestRecord) {
  return [
...
export async function notifyStageApprovers(
  request: BreakInRequestRecord,
  status: Extract<BreakInStatus, "SUBMITTED" | "COORD_REVIEW" | "SUPER_REVIEW" | "MANAGER_REVIEW">,
  triggeredBy: string,
) {
      ...
      return `
        <tr>
          ...
          <td style="padding: 8px 0; border-top: 1px solid #e5e7eb;">
            <div><a href="${approveUrl}">Approve request</a></div>
            <div style="margin-top: 6px;"><a href="${rejectUrl}">Reject request</a></div>
          </td>
        </tr>
      `;
    })
  ...
  return sendEmail({
    ...
    html: `
      <div style="font-family: Arial, Helvetica, sans-serif; color: #111; line-height: 1.5;">
        ...
        <p style="margin-top: 14px; color: #475569; font-size: 12px;">A confirmation page will open after you click an action link.</p>
      </div>
    `,
    text: [
      ...
      ...recipients.flatMap((recipient) => {
        ...
        return [
          `${recipient} approve: ${approveUrl}`,
          `${recipient} reject: ${rejectUrl}`,
        ];
      }),
    ].join("\n"),
  });
...
export async function notifyRequestorOutcome(
  request: BreakInRequestRecord,
  outcome: Extract<BreakInStatus, "APPROVED" | "REJECTED">,
  comment: string,
  decidedBy: string,
) {
